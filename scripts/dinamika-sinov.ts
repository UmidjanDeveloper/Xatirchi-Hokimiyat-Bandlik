...
const SINOVLAR: Sinov[] = [
  ...
  /* ── ЁРЛИҚЛАР: рақамни КУН деб ўқиб бўлмасин ──
   *
   * Илгари ойлик ёрлиқ «Сен 26» эди — ва ўн олтинчи сентябрда
   * панелни очган одам уни «26-сентябрь» деб ўқиди. Кунлик
   * кесимда ёрлиқ айнан шундай, «16 Сен» бўлиб чиқади: иккиси
   * бир-бирининг кўзгуси эди.
   */
  {
    nomi: 'Ойлик ёрлиқда икки рақамли йил ЙЎҚ',
    tekshir: () => {
      const y = dinamikaHisobla([], [], [], 'oy').map((n) => n.yorliq);
      /* «Сен 26» каби — ой номи ва иккита рақам — бўлмаслиги керак */
      return y.every((q) => !/^\D+\s\d{2}$/.test(q));
    },
  },
  {
    nomi: 'Ойлик ёрлиқда йил ТЎЛИҚ ва фақат ўзгарган жойда',
    tekshir: () => {
      const y = dinamikaHisobla([], [], [], 'oy').map((n) => n.yorliq);
      const yilli = y.filter((q) => /\s\d{4}$/.test(q));
      /*
       * Ўн икки ойда йил кўпи билан икки марта ёзилади:
       * биринчи ёрлиқ ва январь. Камида битта бўлиши шарт —
       * акс ҳолда қайси йил экани умуман кўринмайди.
       */
      return yilli.length >= 1 && yilli.length <= 2 && /\s\d{4}$/.test(y[0]);
    },
  },
  {
    nomi: 'Кунлик ёрлиқ ой номини фақат ой бошида такрорлайди',
    tekshir: () => {
      const y = dinamikaHisobla([], [], [], 'kun').map((n) => n.yorliq);
      const oyli = y.filter((q) => q.includes(' '));
      /* Ўттиз кунда ой чегараси кўпи билан иккита бўлади */
      return oyli.length >= 1 && oyli.length <= 2 && y[0].includes(' ');
    },
  },
  {
    nomi: 'Йиллик ёрлиқ — тўрт рақамли йил',
    tekshir: () =>
      dinamikaHisobla([], [], [], 'yil').every((n) => /^\d{4}$/.test(n.yorliq)),
  },
];
